...
use crate::{
    ...
    overrides::Override,
    profiler::{self, FsOperation, ProfileGuard},
    types::Types,
};
...
impl DirEntryRaw {
    ...
    fn metadata_internal(&self) -> Result<fs::Metadata, Error> {
        let _guard = ProfileGuard::new(FsOperation::Metadata);
        if self.follow_link {
    ...
    #[cfg(not(windows))]
    fn metadata_internal(&self) -> Result<fs::Metadata, Error> {
        let _guard = if self.follow_link {
            ProfileGuard::new(FsOperation::Metadata)
        } else {
            ProfileGuard::new(FsOperation::SymlinkMetadata)
        };
        if self.follow_link {
            fs::metadata(&self.path)
        } else {
