...
fn main() -> ExitCode {
    // Check for RG_PROFILE environment variable to enable profiling
    if std::env::var("RG_PROFILE").is_ok() {
        ignore::profiler::enable_profiling();
        eprintln!("Profiling enabled. Results will be written to ./rg_profile/");
    }

    let exit_code = match run(flags::parse()) {
        Ok(code) => code,
        Err(err) => {
    ...
    // Dump profiling data before exit if profiling is enabled
    if ignore::profiler::is_enabled() {
        let profile_dir = std::path::Path::new("./rg_profile");
        if let Err(e) = std::fs::create_dir_all(profile_dir) {
            eprintln!("Warning: Failed to create profile directory: {}", e);
        } else if let Err(e) = ignore::profiler::dump_thread_profile(profile_dir) {
            eprintln!("Warning: Failed to dump profile data: {}", e);
        } else {
            eprintln!("Profile data written to ./rg_profile/");
        }
    }
